...
class RainbowSix(commands.Cog):
    ...
    async def r6stat(self, ctx, username=None, region=None, platform=None):
        ...
        try:
            ...
            # rank = await player.load_rank('ASIA')
            ...
            embed.add_field(name='Current Rank', value='API BORKED', inline=False)
            embed.add_field(name='Overall Stats', value=f"K/D: {round(kd_overall, 2)}\nWin%: {round(win_overall)}", inline=False)
            embed.add_field(name='Ranked Stats', value=f"K/D: {round(kd_ranked, 2)}\nWin%: {round(win_casual)}", inline=False)
            ...
            self.auth.close()
            await ctx.send(f'> Found no player under the name [{username}]')
